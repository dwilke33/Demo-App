    ...
    df = pd.DataFrame(
        {
            ...
            # NEW FIELDS for completion likelihood
...
# -----------------------------
# Rule-based completion likelihood scoring
# -----------------------------
...
def compute_completion_score(df: pd.DataFrame):
    ...
    Uses:
    - DistanceToClinic_km (closer = more likely to complete)
    - Age (mid-age adults often more stable)
    - ComorbidityIndex (fewer comorbidities = more likely)
    - Smoker (non-smokers more adherent on average)

    Returns a new DataFrame with:
    - CompletionScore (0–100)
    - CompletionExplanation (JSON string with rule trace)
    """
    scores = []
    explanations = []
    ...
    for _, row in df.iterrows():
        ...
        if pd.isna(age):
            ...
            if 30 <= age <= 65:
                s += 25
                reasons.append(f"+ prime working/adult age (30–65, age={age})")
            elif 18 <= age < 30:
                s += 15
...
4. Applies a transparent, rule-based scoring system to rank patients by eligibility (**MatchScore**)
5. Estimates likelihood of study completion (**CompletionScore**)
6. Provides a simple chatbot-style interface that uses the same engine
"""
)
...
st.dataframe(view[cols_to_show], use_container_width=True, height=350)
...
if st.button("Send", type="primary"):
    ...
    if user_text:
        ...
        # Top 5 patients by MatchScore primarily (you could also sort by composite)
        chat_scored["FinalCompositeScore"] = np.round(
            0.7 * chat_scored["MatchScore"] + 0.3 * chat_scored["CompletionScore"],
            1,
        )
        chat_scored = chat_scored.sort_values("MatchScore", ascending=False)
...
st.info(
    ...
    "but wraps it in a conversational experience. It returns both **MatchScore** (eligibility) "
    "and **CompletionScore** (likelihood to complete) for a small set of top candidates."
)
